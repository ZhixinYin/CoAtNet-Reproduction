...
## Discussion
### Overfitting Problem
From the result, we can see that the model still has overfitting problems on Tiny ImageNet Dataset. For comparison, ConvNeXt we trained earlier has 78.81% and 60.17% accuracy. One of the reasons is that Tiny ImageNet only has 100k images. There aren't enough samples to rule out bad hypothesis and spurious patterns are not likely to get contradicted during training. Another reason is that transformer blocks can correlate any two patches in a layer, which helps the model memorise the patterns. Although it has MBConv blocks at early stages, it only recognise low-level patterns and add some inductive bias. Once it reaches transformer stages, the model starts memorising patterns, as we don't have enough samples to rule out the spurious patterns. Consequently, our model has overfitting problems.
